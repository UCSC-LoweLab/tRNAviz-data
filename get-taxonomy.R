...
genome_table = genome_table %>% left_join(taxonomy %>% filter(assembly != ''), by = 'taxid') %>%
  select(dbname, taxid, name, kingdom, subkingdom, phylum, subphylum, class, subclass, order, family, genus, species, assembly)
...
genome_table = genome_table %>% filter(taxid %in% ncbi_refs$V7 | taxid %in% approved_taxids)
