...
def main():
  ...
  trnas = trnas[trnas.genus == 'Saccharomyces']
  # taxonomy = pd.read_csv(taxonomy, sep = '\t')
  print(resolve_consensus_isotypes(trnas))
...
def parse_args():
  ...
  parser.add_argument('-f', '--clade_isotype_freqs', default = 'clade-isotype-freqs-{}.tsv'.format(timestamp), help = '')
  parser.add_argument('-c', '--clean', default = True, action = 'store_false', help = 'Attempt to automatically clean up intermediate files')
