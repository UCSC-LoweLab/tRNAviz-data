...
def main():
  ...
  for clade in ranks.keys():
    message('\tCounting base frequencies for {} {}...'.format(ranks[clade], clade))
    ...
    current_clade_freqs['clade'] = clade
    current_clade_freqs['rank'] = ranks[clade]
    freqs = freqs.append(current_clade_freqs, sort = False)
    message('done\n')
  message('Done\n')
  
  message('Exporting results to {}...'.format(output_file))
  colorder = ['isotype', 'clade', 'rank', 'position', 'A', 'C', 'G', 'U', '-', 'A:U', 'U:A', 'G:C', 'C:G', 'G:U', 'U:G', '-:-']
  freqs = freqs.reindex(columns = colorder + [col for col in freqs.columns if col not in colorder])
...
def count_freqs_isotypes(trnas):
  ...
  for isotype in isotypes:
    ...
    freqs = freqs.append(current_isotype_freqs, sort = False)
  return(freqs)
...
def count_freqs(trnas):
  ...
  for position in positions.values():
    current_position = trnas.loc[:, position].value_counts()
